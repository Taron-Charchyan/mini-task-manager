...
export const fetchTasks = (page, limit) => {
    return async (dispatch) => {
        ...
        try {
            ...
            dispatch({ type: FETCH_TASKS_ERROR, payload: err.response?.data?.message || err.message });
...
export const editTask = (id, editedTask) => {
    return async (dispatch) => {
        ...
        try {
            ...
            dispatch({ type: EDIT_TASKS_SUCCESS, payload: data });
            toast.success("Task edited successfully.");
        } catch (err) {
            dispatch({ type: EDIT_TASKS_ERROR, payload: err.response?.data?.message || err.message });
